...
export default function Home() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#ffffff", fontFamily: "Inter, Arial, sans-serif" }}>
      ...
      {/* Flows + Feature Line */}
      <section style={{ display: "flex", justifyContent: "center", alignItems: "flex-start", gap: 20, maxWidth: 1400, margin: "0 auto", padding: "0 16px" }}>
        {/* Personal Flow */}
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", width: "25%" }}>
          ...
          {personalSteps.map((item, idx) => (
            <div key={idx} style={cardStyle("personal")}
            ...
            >
              ...
              {idx < personalSteps.length -1 && <div style={{ fontSize: 20, color: "#1f6feb" }}>→</div>}
            </div>
          ))}
        </div>

        {/* Feature center line */}
        <div style={{ width: "30%", textAlign: "center", marginTop: 50 }}>
          <p style={{ fontSize: "1.2rem", fontWeight: 500, color: "#0d1f4c", whiteSpace: "nowrap" }}>
            📊 Visualize your financial health with charts
          </p>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", width: "25%" }}>
          ...
          {proSteps.map((item, idx) => (
            <div key={idx} style={cardStyle("pro")}
            ...
            >
              ...
              <h3 style={{ marginBottom: 6 }}>{item.title}</h3>
              <p style={{ fontSize: 13, color: "#555" }}>{item.desc}</p>
              {idx < proSteps.length -1 && <div style={{ fontSize: 20, color: "#0ea5a0" }}>→</div>}
            </div>
          ))}
        </div>
      </section>
